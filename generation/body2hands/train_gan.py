...
def main(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        if epoch > 0 and epoch % 3 == 0:
            ...
            currBestLoss, testLoss = val_generator(args, generator, discriminator, reg_criterion, g_optimizer, test_X, test_Y, currBestLoss, test_ims=test_ims)
            loss_list.append(testLoss)

        if epoch % 100 == 0:
            timestep = range(1,len(loss_list)+1)
            plt.figure(figsize=(10,6))
            plt.plot(timestep, loss_list)
            min_index = min(range(len(loss_list)), key=loss_list.__getitem__)
            plt.plot(min_index+1, loss_list[min_index], 'rx', markersize=20)
...
def load_data(args, rng):
    ...
    curr_p0, curr_p1, _ = load_windows(DATA_PATHS, args.pipeline, require_image=args.require_image)
...
### python3 train_gan.py --model models/ --base_path ./ --num_epochs 100000