...
def load_windows(data_dir, pipeline, num_samples=None, use_euler=False, require_image=False, require_audio=False, hand3d_image=False, use_lazy=False, test_smpl=False, temporal=False):
    ...
    if os.path.exists(os.path.join(data_dir, 'nc_train_body.npy')):
        print('using super quick load', data_dir)
        p1_windows = np.load(os.path.join(data_dir, 'nc_train_hands.npy'), allow_pickle=True)
        p0_windows = np.load(os.path.join(data_dir, 'nc_train_body.npy'), allow_pickle=True)
